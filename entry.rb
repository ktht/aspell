...
files.each do |file|
  ...
  if errors.empty?
    ...
    errors.each do |word|
      message = <<~EOF
        Wrong spelling of “#{word[:word]}” found (line #{word[:line]}, column #{word[:column]}). Maybe you meant one of the following?

        #{word[:suggestions].join(', ')}
      EOF

      puts "::error file=#{escape(file)},line=#{word[:line]},col=#{word[:column]}::#{escape(message)}"
    end
